...
void main() {
  group('NotificationEntity', () {
    ...
    group('formattedDate getter', () {
      test('returns "Today" for notifications created today', () {
        final todayNotification = notification.copyWith(
          createdAt: DateTime.now().subtract(const Duration(hours: 2)),
        );
        expect(todayNotification.formattedDate, 'Today');
      });

      test('returns "Yesterday" for notifications created yesterday', () {
        final yesterdayNotification = notification.copyWith(
          createdAt: DateTime.now().subtract(const Duration(days: 1)),
        );
        expect(yesterdayNotification.formattedDate, 'Yesterday');
      });

      test('returns "X days ago" for notifications less than 7 days old', () {
        final threeDaysAgoNotification = notification.copyWith(
          createdAt: DateTime.now().subtract(const Duration(days: 3)),
        );
        expect(threeDaysAgoNotification.formattedDate, '3 days ago');
      });

      test('returns date in M/D/YYYY format for notifications 7+ days old', () {
        final oldNotification = notification.copyWith(
          createdAt: DateTime(2025, 10, 1),
        );
        expect(oldNotification.formattedDate, '10/1/2025');
      });
    });
    ...
    group('requiresAction getter', () {
      ...
      test('returns true for mention notifications', () {
        final mentionNotification =
            notification.copyWith(type: NotificationType.mention);
        expect(mentionNotification.requiresAction, true);
      });
    ...
    group('actionButtonText getter', () {
      ...
      test('returns "View" for mention notifications', () {
        final mentionNotification =
            notification.copyWith(type: NotificationType.mention);
        expect(mentionNotification.actionButtonText, 'View');
      });

      test('returns null for other notification types', () {
        final likeNotification =
            notification.copyWith(type: NotificationType.like);
        expect(likeNotification.actionButtonText, null);

        final commentNotification =
            notification.copyWith(type: NotificationType.comment);
        expect(commentNotification.actionButtonText, null);

        final shareNotification =
            notification.copyWith(type: NotificationType.share);
        expect(shareNotification.actionButtonText, null);
      });
    });
