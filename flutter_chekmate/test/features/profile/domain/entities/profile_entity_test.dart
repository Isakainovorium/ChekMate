  ...
  group('ProfileEntity', () {
    ...
    group('Business Logic Methods', () {
      ...
      test('isComplete returns false when age is missing', () {
        final incompleteProfile = ProfileEntity(
          uid: testProfile.uid,
          ...
          following: testProfile.following,
          posts: testProfile.posts,
          isVerified: testProfile.isVerified,
          isPremium: testProfile.isPremium,
      ...
      test('hasVideoIntro returns false when video intro is null', () {
        final noVideoProfile = ProfileEntity(
          uid: testProfile.uid,
          username: testProfile.username,
          displayName: testProfile.displayName,
          bio: testProfile.bio,
      ...
      test('completionPercentage returns 0.875 when one field is missing', () {
        final incompleteProfile = testProfile.copyWith(bio: '');
        expect(incompleteProfile.completionPercentage, 0.875); // 7/8
      });

      test('completionPercentage returns 0.75 when two fields are missing', () {
        final incompleteProfile = testProfile.copyWith(bio: '', avatar: '');
        expect(incompleteProfile.completionPercentage, 0.75); // 6/8
      });

      test('canBeMessaged returns true for non-premium user', () {
        expect(testProfile.canBeMessaged, true); // !isPremium = true
      });

      test('canBeMessaged returns false for premium unverified user', () {
        final premiumUnverified =
            testProfile.copyWith(isPremium: true, isVerified: false);
        expect(
            premiumUnverified.canBeMessaged, false); // !true || false = false
      });
    });
    ...
    group('Equality', () {
      test('two profiles with same properties are equal', () {
        ...
        final profile2 = ProfileEntity(
          ...
          location: testProfile.location,
          interests: testProfile.interests,
          voicePrompts: testProfile.voicePrompts,
          videoIntroUrl: testProfile.videoIntroUrl,
          email: testProfile.email,
          stats: testProfile.stats,
        );
        expect(profile1, equals(profile2));
      });

      test('two profiles with different properties are not equal', () {
        final profile1 = testProfile;
        final profile2 = testProfile.copyWith(bio: 'Different bio');
        expect(profile1, isNot(equals(profile2)));
      });

      test('hashCode is consistent for same properties', () {
        final profile1 = testProfile;
        final profile2 = ProfileEntity(
          uid: testProfile.uid,
          username: testProfile.username,
